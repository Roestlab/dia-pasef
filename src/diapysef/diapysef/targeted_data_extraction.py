...
def method_timer(f):
    ...
    def wrap(*args, **kw):
        ...
        result = f(*args, **kw)
        te = time()
        logging.debug('method:%r args:[%r, %r] took: %2.4f sec' % (f.__name__, args, kw, te-ts))
        return result
    return wrap
...
def setup_logger(log_file, verbose):
    ...
    logging.basicConfig(level=use_verbosity, filename=log_file, filemode='w', format='[%(asctime)s] %(name)s - %(levelname)s - %(message)s')
...
def generate_coordinates(file, outfile=None, run_id=None, target_peptides=None, m_score=0.05, use_transition_peptide_mapping=False, use_only_detecting_transitions=True, verbose=0):
  ...
  if file.lower().endswith("osw"):
    if verbose==10:
      logging.debug(f"Connecting to OSW identifications database: {file}")
    ...
    if target_peptides is not None:
      ...
      if os.path.isfile(target_peptides):
        logging.info(f"Reading target peptides from file: {target_peptides}")
        with open(target_peptides, 'r') as file:
          data = file.read().replace('\n', '')
        target_peptides = [peptide.strip() for peptide in data.split(',')]

      logging.info(f"Generating coordinates for the following peptides: {target_peptides}")
      ...
      data = data.loc[data.groupby('group_id')['qvalue'].transform('min').eq(data['qvalue'])].reset_index(drop=True)
      
      # Group and aggregate product m/z into a list
      agg_product_mz_df = data.groupby(['group_id'])['product_mz'].apply(list).to_frame().reset_index()
      data = pd.merge(data.drop('product_mz', axis=1), agg_product_mz_df, on=['group_id'])

      # Aggegate left and right bounaries into a list
      data['rt_boundaries'] = data[['left_width', 'right_width']].values.tolist()
      ...
      peptide_coordinates_dict = data_long.groupby('group_id')[['variable','value']].apply(lambda x: dict(x.to_numpy())).to_dict()
  ...
  if outfile is not None:
    ...
    with open(outfile, "wb") as output_file: pkl.dump(peptide_coordinates_dict, file=output_file )
  else:
...
class data_io():
  ...
  def load_data(self, is_filtered=False):
    ...
    if is_filtered:
      ...
      logging.info(f"There are {exp.getNrSpectra()} spectra and {exp.getNrChromatograms()} chromatograms.")
      logging.info(f"There are {sum([spec.getMSLevel()==1  for spec in exp.getSpectra()])} MS1 spectra and {sum([spec.getMSLevel()==2  for spec in exp.getSpectra()])} MS2 spectra.")
      ...
      logging.info(f"There are {meta_data.getNrSpectra()} spectra and {exp.getNrChromatograms()} chromatograms.")
      logging.info(f"There are {sum([spec.getMSLevel()==1  for spec in meta_data.getSpectra()])} MS1 spectra and {sum([spec.getMSLevel()==2  for spec in meta_data.getSpectra()])} MS2 spectra.")
  ...
  def get_filtered_df(self, ms_level=[1]):
    ...
    for k in range(self.filtered.getNrSpectra()):
       ...
       if spec.getMSLevel() in ms_level:
           ...
           add_df = pd.DataFrame({'native_id':spec.getNativeID(), 'ms_level':spec.getMSLevel(), 'peptide':spec.getMetaValue('peptide'), 'mz':mz, 'rt':rt, 'im':im, 'int':intensity})
    ...
    for k in range(self.filtered.getNrSpectra()):
      ...
      if spec.getMSLevel() in ms_level:
          ...
          if (len(mz)==0 and len(intensity)==0):
              logging.warn(f"Spectrum native id {spec.getNativeID()} had no m/z or intensity array, skipping this spectrum")
              continue
          rt = np.full([mz.shape[0]], spec.getRT(), float)
          str_im = spec.getStringDataArrays()[0]
          im = np.array([float(s) for s in str_im]).astype(np.float32)
          ...
          if self.verbose==10:
              logging.debug(f"Adding MS{spec.getMSLevel()} spectrum for peptide: {spec.getMetaValue('peptide')} with native id: {spec.getNativeID()}")
          add_df = pd.DataFrame({'native_id':spec.getNativeID(), 'ms_level':spec.getMSLevel(), 'peptide':spec.getMetaValue('peptide'), 'precursor_mz':precursor.getMZ(), 'charge':precursor.getCharge(), 'mz':mz, 'rt':rt, 'im':im, 'int':intensity})
          results_df = pd.concat([results_df, add_df])
    logging.info(f"Saving filtered target spectra data to tabular tsv format: {out_file}")
  ...
  def get_target_ms_level_indices(self):
    ...
    mslevel_indices = np.array([indice for indice, spec in enumerate(spectra) if spec.getMSLevel() in self.mslevel])
...
class TargeteddiaPASEFExperiment(data_io):
  ...
  def __init__(self, mzml_file, peptides, mz_tol=20, rt_window=50, im_window=0.06, mslevel=[1], verbose=0, log_file='diapasef_data_extraction.log', threads=1):
    ...
    """
    super().__init__(mzml_file, mz_tol, rt_window, im_window, mslevel, verbose, log_file)
    ...
    if spec.getRT() >= rt_start and spec.getRT() <= rt_end:
      ...
      for mz, i , im in zip(*spec.get_peaks(), spec.getFloatDataArrays()[0].get_data()):
        if spec.getMSLevel() in mslevel and mz > target_precursor_mz_lower and mz < target_precursor_mz_upper and im >= im_start and im <= im_end:
          if verbose==10:
            logging.debug(f"Adding MS1 spectrum {spec.getNativeID()}")
          ...
          if verbose==10:
            logging.debug(f"Adding MS2 spectrum {spec.getNativeID()}")
          filtered_mz.append(mz)
          filtered_int.append(i)
          filtered_im.append(im)
      ...
      spec.setMetaValue('peptide', self.peptides[target_peptide_group]['peptide'])
      
      precursor = po.Precursor()
      precursor.setCharge(self.peptides[target_peptide_group]['charge'])
      precursor.setMZ(self.peptides[target_peptide_group]['precursor_mz'])
  ...
  def reduce_spectra(self, output_spectra_file=None):
    ...
    if output_spectra_file is not None:
        logging.debug(f"Initializing data consumer to write data to disk - {output_spectra_file}")
    ...
    for target_peptide_group in self.peptides.keys():
        ...
        target_precursor_mz_lower, target_precursor_mz_upper  = self.get_upper_lower_tol(target_precursor_mz)
        target_product_upper_lower_list = [self.get_upper_lower_tol(mz) for mz in target_product_mz]
        
        rt_start, rt_end = self.get_rt_upper_lower(rt_apex)
        im_start, im_end = self.get_im_upper_lower(im_apex)
        
        logging.info(f"Extracting data for {target_peptide_group} | target precursor: {target_precursor_mz} m/z ({target_precursor_mz_lower} - {target_precursor_mz_upper}) | RT: {rt_apex} sec ({rt_start} - {rt_end}) | IM: {im_apex} 1/Ko ({im_start} - {im_end})")
        
        # Restrict spectra list further for RT window, to reduce the number of spectra we need to check to perform filtering on
        use_rt_spec_indices = np.where(np.array([spec_rt >= rt_start and spec_rt <= rt_end for spec_rt in self.meta_rt_list]))
        target_spectra_indices = np.intersect1d(self.mslevel_indices, use_rt_spec_indices)
        ...
        with code_block_timer(f"Filtering Spectra..."):
          try:
            ...
            # TODO: Currently very memory heavy, maybe save filtered spectrums to disk?
            filt_spec_list = Parallel(n_jobs=1)(delayed(self.filter_single_spectrum)(spectrum_indice, self.mslevel, target_peptide_group, rt_start, rt_end, im_start, im_end, target_precursor_mz_lower, target_precursor_mz_upper, target_product_upper_lower_list, self.verbose) for spectrum_indice in target_spectra_indices.tolist())
          except:
            traceback.print_exc(file=sys.stdout)
        
        # Add filtered spectra to MSExperiment container
        if self.consumer is None:
            filt_spec_list = list(filter(lambda spectra: spectra is not None, filt_spec_list))
