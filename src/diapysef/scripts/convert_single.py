...
def store_frame(frame_id, td, conn, exp, verbose=False, compressFrame=True):
    ...
    print (scandata, "next switch", next_scan_switch)
    ...
    # for k, scan in list(enumerate(td.readScans(frame_id, 0, num_scans))):
    for k, scan in reversed(list(enumerate(td.readScans(frame_id, 0, num_scans)))):
        ...
        if compressFrame:
            ...
            if next_scan_switch != -1 and next_scan_switch == k:
                ...
                if in_scan:
                    # Only store spectrum when actually inside a scan
                ...
                if in_scan:
                    ...
                    in_scan = False
                    next_scan_switch = scan_end
                    if next_scan_switch == k: next_scan_switch += 1 # if start/end are equal!
                    print("will leave scan now, next scan starts at:", next_scan_switch)
                else:
                    in_scan = True
                    next_scan_switch = scan_start
                    print("in scan now, scan will end at !", next_scan_switch)
...
def main():
    ...
    for frame_id in range(N):
        store_frame(frame_id+1, td, conn, consumer, compressFrame=True, verbose=True)
